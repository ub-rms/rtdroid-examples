 ...
 * 
 * FIXME AbstractFLightPlan should inherit from this method! NOTE: this file is
 * partly based on Paparazzi code (method prefix nav) and partly on Papabench
 * code.
 * 
...
public abstract class NavigatorCommands {
  ...
  /**
   * Set the climb control to auto-throttle with the specified pitchpre-command
   */
  ...
  protected final void navVerticalClimbMode(float climb) {
    ...
    status().setVerticalFlightMode(VerticalFlightMode.AUTO_CLIMB); // AUTO_CLIMB
                                                                   // ->
                                                                   // navigator
                                                                   // has to
                                                                   // obtain
  ...
  /**
   * Decide if the UAV is approaching the current waypoint. Computes dist2_to_wp
   * and compare it to square carrot. Return true if it is smaller. Else
   * computes by scalar products if uav has not gone past waypoint. Return true
   * if it is the case.
   */
  protected final boolean navApproachingFrom(int toWP, int fromWP,
      float approachingTime) {
    return navApproachingFrom(WP(toWP), WP(fromWP), approachingTime);
  }

  protected final boolean navApproachingFrom(Position3D toWP,
      Position3D fromWP, float approachingTime) {
    ...
    float scalarProduct =
        MathUtils.scalarProduct(toWP, fromWP, estimator().getPosition());
  ...
  protected final void navRouteXY(Position3D startWP, Position3D endWP) {
    ...
    float navLegProgress =
        ((estimator().getPosition().x - startWP.x) * legX + (estimator()
            .getPosition().y - startWP.y) * legY)
            / leg2;
    ...
    //
    // flyToXY(startWP.x+navLegProgress*legX + navShift*legY/navLegLength,
    // startWP.y+navLegProgress*legY-navShift*legX/navLegLength);
  ...
  protected final boolean approaching(Position3D toWP, Position3D fromWP) {
    ...
    carrot = carrot < 40 ? 40 : carrot; // FIXME what is number 10 (40 in
                                        // original version)? replace by
                                        // constant with user friendly name
  ...
  // WARNING - this method is implemented according to PapaBench code (it is not
  // compatible with paparazzi)
  protected final void flyToXY(float x, float y) {

    navigator().setDesiredPosition(x, y);
    navigator().setDesiredCourse(
        (float) (Math.PI / 2 - Math.atan2(y - estimator().getPosition().y, x
            - estimator().getPosition().x)));
    // LogUtils.log(this, "FlyToWP: (" + x + ", "+ y + "), course = " +
    // Math.toDegrees(navigator().getDesiredCourse()));
  }
  ...
  protected final void routeTo(Position3D fromWP, Position3D toWP) {
    ...
    float leg2 = Math.max(legX * legX + legY * legY, 1f);
    float alpha =
        ((estimator().getPosition().x - fromWP.x) * legX + (estimator()
            .getPosition().y - fromWP.y) * legY)
            / leg2;
