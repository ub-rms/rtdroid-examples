...
public class Simple06FlightPlan extends AbstractFlightPlan {
  ...
  protected void initNavigationBlocks() {
    ...
    addNavBlock(new NavigationBlock(3)).addNavStage(new NavigationStage() {
      ...
      @Override
      protected void execute() {
        //System.out.println("Estimator Z=" + estimator.getPosition().z);
    ...
    /* navigation block 02 */
    addNavBlock(new NavigationBlock(3)).addNavStage(new NavigationStage() { // stage
                                                                            // 0
          @Override
          protected void execute() {
            if (approaching(1)) {
//              LogUtils.log(this, "Current position: " + estimator.getPosition());
//              LogUtils.log(this, "WP 1 " + WP(1).toString()
//                  + " reached! Going to the next stage");
              nextStageFrom(1);
            } else {
              flyToWP(1); // 0, 0, 200
              navigator().setAutoPitch(false);
          ...
          protected void execute() {
            if (approaching(4)) {
//              LogUtils.log(this, "Current position: " + estimator.getPosition());
//              LogUtils.log(this, "WP 4 " + WP(4).toString()
//                  + " reached! Going to the next stage");
              nextStageFrom(4);
            } else {
              flyToWP(4); // 115,0,200
