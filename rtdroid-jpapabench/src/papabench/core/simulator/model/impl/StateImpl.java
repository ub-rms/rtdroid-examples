...
public class StateImpl implements State {
  ...
  public void updateState(float dt, Position3D wind) {
    ...
    if (airSpeed > 0) {
      ...
      float phiDotDot =
          ROLL_RESPONSE_FACTOR * delta.x * v2 / vn2 - rotSpeed.phi;
      this.rotSpeed.phi += phiDotDot * dt;
      this.rotSpeed.phi =
          MathUtils.symmetricalLimiter(this.rotSpeed.phi,
              SimulatorConf.MAX_PHI_DOT);
      this.attitude.phi =
          MathUtils.normalizeRadAngle(this.attitude.phi + this.rotSpeed.phi
              * dt);
      this.attitude.phi =
          MathUtils
              .symmetricalLimiter(this.attitude.phi, SimulatorConf.MAX_PHI);

      float psiDot =
          (float) (-G / airSpeed * Math.tan(YAW_RESPONSE_FACTOR * attitude.phi));

      this.attitude.psi =
          MathUtils.normalizeRadAngle(this.attitude.psi + psiDot * dt);
      ...
      float cZ = 0.2f * alpha + AirframeParametersConf.G / vn2; // FIXME why is
                                                                // there
                                                                // constant 0.2
                                                                // (copied from
                                                                // paparazzi
      ...
      float zDotDot =
          (float) (lift / WEIGHT * Math.cos(this.attitude.theta)
              * Math.cos(this.attitude.phi) - AirframeParametersConf.G);
      this.zDot += zDotDot * dt;
      this.position.z += zDot * dt;

      float drag =
          CRUISE_THROTTLE + (v2 - vn2) * (1 - CRUISE_THROTTLE)
              / (MAXIMUM_AIRSPEED * MAXIMUM_AIRSPEED - vn2);
      float airSpeedDot =
          (float) (MAXIMUM_POWER / this.airSpeed * (this.thrust - drag)
              / WEIGHT - G * Math.sin(gamma));
  ...
  public void updateState(RadioCommands commands) {
    ...
    // LogUtils.log(this, "Radio commands for simulator state update: " +
    // commands.toString());
    this.delta.x = -cLda * commands.getRoll();
    this.delta.y = commands.getPitch();
    this.thrust =
        (commands.getThrottle() - MIN_THRUST)
            / (float) (MAX_THRUST - MIN_THRUST);
    // LogUtils.log(this, "State Updated: delta.x="+delta.x +
    // ", delta.y="+delta.y +", thrust="+thrust);
  }
  ...
  public String toString() {
    return "StateImpl [airSpeed=" + airSpeed + ", attitude=" + attitude
        ...
        + rotSpeed + ", thrust=" + thrust + ", time=" + time + ", zDot=" + zDot
        + "]";
  }
}
