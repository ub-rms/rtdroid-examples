...
/**
 * Simulated GPS device implementation. It provides data computed according last
...
public class SimulatorGPSDeviceImpl implements GPSDevice, SimulatedDevice {
  ...
  public void update(FlightModel flightModel) {
    update(flightModel.getState().getPosition(), /* FIXME */125, flightModel
        .getState().getTime());
  }

  // FIXME should reflects initial UTM position of airplane : now initial
  // position is (0,0). The same for altitude
  protected void update(Position3D position, float groundAltitude, float time) {
    ...
    if (dt > 0f) {
      ...
      lastCourse =
          MathUtils.normalizeRadAngle((float) (Math.PI - Math.atan2(dy, dx)));
      lastClimb = (position.z - lastPosition.z) / dt;
    }

    // FIXME following code transforms (x,y) position to UTM position reflecting
    // initial position
    ...
    course =
        (float) (lastCourse < 0 ? lastCourse + MathUtils.TWO_PI : lastCourse);
