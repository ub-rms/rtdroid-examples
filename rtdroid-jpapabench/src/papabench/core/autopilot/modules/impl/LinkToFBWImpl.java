 ...
 * 
 * Notes: - direct link is only for testing purposes - it should be replaced by
 * code handling SPI bus
 * 
...
public class LinkToFBWImpl implements LinkToFBW {
  ...
  public void init() {
    if (spiBus == null) {
      throw new IllegalArgumentException(
          "FBWLink module is not configured correctly!");
    }
  }
  ...
  public InterMCUMsg getMessageFromFBW() {
    ...
    // async receive -if the message is not fully received the message is marked
    // as not-valid
    // The caller is in this case task, so the message will be allocated in the
    // scope of caller task.
