 ...
 * sensors.
 * 
 * Notes: - internal data are allocated in the same memory as this module.
 * 
...
public class EstimatorModuleImpl implements Estimator, Module {
  ...
  protected static final float RHO = 0.999f; /*
                                              * The higher, the slower the
                                              * estimation is changing
                                              */
  protected static final float INIT_WEIGHT = 100.0f; /*
                                                      * The number of times the
                                                      * initial value has to be
                                                      * taken
                                                      */
  ...
  /*
   * airplane attitude in radians x -> + = right (PHI) y -> CW, 0 = N (PSI) z ->
   * + = up (THETA) theta = The pitch in degrees relative to a
   * plane(mathematical) normal to the earth radius line at the point the
   * plane(aircraft) is at phi = The roll of the aircraft in degrees psi = The
  ...
  private HorizSpeed horizontalSpeed; // (estimator_hspeed_mod,
  ...
  public void init() {
    if (gpsDevice == null || irDevice == null) {
      throw new IllegalArgumentException(
          "Estimator modules is not properly configured");
    }
  }
  ...
  public void updateIR() { // -> estimator_update_ir_estim
    if (irInitialized) {
      ...
      if (dt > 0.1) {
        ...
        phi =
            phi / dt * AirframeParametersConf.NOMINAL_AIRSPEED
                / AirframeParametersConf.G;
        phi = MathUtils.normalizeRadAngle(phi);
        ...
        if (absphi < 1.0f
            && absphi > 0.05f
            && (-irDevice.getIrContrast() / 2 < irDevice.getIrRoll() && irDevice
                .getIrRoll() < irDevice.getIrContrast() / 2)) {
  ...
  public void updatePosition() {
    if (true) { // FIXME
      ...
      updatePosition(gpsDevice.getEast(), gpsDevice.getNorth(),
          gpsDevice.getAltitude());
      updateSpeedPol(gpsDevice.getSpeed(), gpsDevice.getCourse(),
          gpsDevice.getClimb());
