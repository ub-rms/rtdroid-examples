...
/**
 * Computes desired_aileron and desired_elevator from attitude estimation and
 * expected attitude.
 * 
...
public class RollPitchPIDController extends AbstractPIDController {
  ...
  public void control(AutopilotStatus status, Estimator estimator,
    ...
    err =
        -(estimator.getAttitude().theta - status.getPitch() - status
            .getPitchOfRoll() * Math.abs(estimator.getAttitude().phi));

    int desiredElevator =
        (int) PPRZUtils.trimPPRZ(status.getPitchPGain() * err);
