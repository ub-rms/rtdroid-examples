...
import papabench.android.FBWService;
...
//    ((FBWService) fbwModule.getService()).sendMsgToAutopilot(
//        AutopilotService.MSG_NON_CRITICAL, "msg from fbw");

    // send only valid
    if (fbwModule.getPPMDevice().isValid()) {
      InterMCUMsg msg = new InterMCUMsg();
      msg.radioCommands =
          fbwModule.getPPMDevice().getLastRadioCommands().clone();
